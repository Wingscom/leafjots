...
files_modified:
  ...
  - alembic/versions/phase12_taxable_transfers.py
autonomous: true
requirements:
  - ANAL-01
  - ANAL-02
...
<task type="auto">
  <name>Task 2: Create TaxAnalyticsRepo + TaxableTransferRecord model + update domain model + update TaxEngine</name>
  <files>
    src/cryptotax/db/repos/tax_analytics_repo.py
    src/cryptotax/db/models/taxable_transfer.py
    src/cryptotax/db/models/__init__.py
  ...
  <action>
**Step 0: Update domain TaxableTransfer model** in `src/cryptotax/domain/models/tax.py`:

Add two missing fields to the `TaxableTransfer` dataclass/model that are needed to populate `TaxableTransferRecord`:
- `value_usd: Decimal` — the USD value of the transfer (available in splits_data as `s["value_usd"]`)
- `journal_entry_id: uuid.UUID` — FK to the journal entry (available in splits_data as `s["journal_entry_id"]`)

Update `_calculate_transfer_tax` in `tax_engine.py` to populate these fields from `splits_data` when creating `TaxableTransfer` objects.
...
Also update `src/cryptotax/db/models/__init__.py`: add `from cryptotax.db.models.taxable_transfer import TaxableTransferRecord` and add `TaxableTransferRecord` to `__all__`. This is REQUIRED for Alembic's `--autogenerate` in Task 3 to detect the new table (alembic/env.py scans `Base.metadata` via this package import).
...
Run: `python -c "from cryptotax.accounting.tax_engine import TaxEngine; print('OK')"` — updated engine imports.
  ...
  <done>TaxableTransferRecord model exists, domain TaxableTransfer updated with value_usd + journal_entry_id, TaxAnalyticsRepo has 8 methods, TaxEngine persists taxable transfers alongside closed/open lots.</done>
</task>

<task type="auto">
  <name>Task 3: Generate Alembic migration for taxable_transfers table</name>
  <files>alembic/versions/phase12_taxable_transfers.py</files>
  <action>
Generate an Alembic migration for the new `taxable_transfers` table:

```bash
cd /d/work/crytax
alembic revision --autogenerate -m "phase12_taxable_transfers"
```

Verify the generated migration contains:
- `op.create_table('taxable_transfers', ...)` with all columns matching TaxableTransferRecord
- Foreign keys to `entities.id` and `journal_entries.id`
- Proper downgrade with `op.drop_table('taxable_transfers')`

Add indexes for common query patterns:
- `ix_taxable_transfers_entity_id` on `entity_id`
- `ix_taxable_transfers_timestamp` on `timestamp`
- `ix_taxable_transfers_symbol` on `symbol`

Run: `alembic upgrade head` to apply the migration.
  </action>
  <verify>
Run: `alembic current` — shows latest migration applied.
Run: `python -c "from sqlalchemy import inspect; from cryptotax.db.session import engine; i = inspect(engine); print('taxable_transfers' in i.get_table_names())"` — or verify via psql that the table exists.
  </verify>
  <done>Alembic migration for taxable_transfers table is generated and applied. Table exists in PostgreSQL with proper columns, foreign keys, and indexes.</done>
</task>
